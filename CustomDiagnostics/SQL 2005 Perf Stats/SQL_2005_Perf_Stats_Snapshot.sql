SET NOCOUNT ON
IF (@@microsoftversion <150997986) BEGIN
  PRINT ''
  PRINT '**** NOTE ****'
  PRINT '**** This script is for SQL Server 2005 sp2 or later.  Errors are expected when run on earlier versions.'
  PRINT '**************'
  PRINT ''
END
ELSE BEGIN
...
SELECT 
  ...
  END, 40) AS dbname,
  sql.objectid, 
	object_name(sql.objectid, sql.dbid) 
 AS procname, 
